"""
YOLO11 Detection Model Egitici
================================
Ultralytics YOLO11 modeli ile object detection egitimi.
"""
...
class YOLO11Trainer(BaseTrainer):
    MODEL_NAME = "yolo11"
    DESCRIPTION = "Ultralytics YOLO11 Detection (n/s/m/l/x)"
    DEFAULT_BATCH_SIZE = 48

    def setup_model(self, **kwargs):
        """YOLO11 modelini yukler."""
        weight = kwargs.get("weight", "yolo11s.pt")
        weight_path = self.get_weight_path(weight)
        self.model = YOLO(weight_path)
        print(f"[+] Model yuklendi: {weight_path}")

    def run_training(self, **kwargs):
        """YOLO11 egitimini baslatir."""
        dataset_yaml = kwargs.get("dataset_yaml")
        if not dataset_yaml:
            raise ValueError("YOLO egitimi icin 'dataset_yaml' parametresi gerekli (data.yaml yolu).")

        experiment_name = kwargs.get("experiment_name", "yolo11_experiment")
